...
Claw-Republic 将通过 **CLI**、**OAuth** 或 **直接 API Key** 连接的 AI 编程助手，转化为一个完整模拟的**虚拟软件公司**。您是 CEO，AI 代理是员工。看着它们跨部门协作、召开会议、完成任务、不断成长 —— 一切都通过迷人的像素风格办公室界面直观呈现。
...
- 文件直链: [`Sample_Img/claw-republic-intro.mp4`](Sample_Img/claw-republic-intro.mp4)
...
无需 OpenClaw，也可以仅用 Claw-Republic 直接配置并运行消息渠道。
...
# 6. 设置 AGENTS.md 编排规则（教 AI 代理成为 Claw-Republic 项目经理）
...
- 调用 Claw-Republic REST API 创建任务、分配代理、报告状态
...
当聊天消息以 `$` 开头时，Claw-Republic 会将其作为 CEO 指令处理：
...
curl -X POST http://127.0.0.1:8790/api/inbox \
  -H "content-type: application/json" \
  -H "x-inbox-secret: $INBOX_WEBHOOK_SECRET" \
  -d '{"source":"telegram","author":"ceo","text":"$立即修复生产环境登录故障","skipPlannedMeeting":true,"project_context":"之前在做的 claw-republic 项目"}'
```
...
| `DB_PATH`                           | 否                           | SQLite 数据库路径（默认：`./claw-republic.sqlite`）                      |
...
| `UPDATE_CHECK_REPO`                 | 否                           | 更新检查使用的 GitHub 仓库标识（默认：`GreenSheep01201/claw-republic`）  |
...
当 GitHub 发布了更高版本时，Claw-Republic 会在 UI 顶部显示包含 pull 操作说明和发布说明链接的横幅。
