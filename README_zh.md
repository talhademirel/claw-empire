...
<p align="center">
  ...
  <a href="docs/releases/v2.0.2.md">发布说明</a> &middot;
...
- **在 CI 中直接阻止 OpenAPI 契约漂移** - `test:ci` 现已包含 `openapi:check`，路由与文档不一致会在 `main` 上直接失败。
- **为关键运维 API 流程补充 E2E 覆盖** - CI 现在直接验证任务运行/停止/注入/恢复、终端与会议纪要读取、`/api/inbox` 指令 webhook、项目路径辅助、CLI 诊断以及 API 提供商 CRUD。
- **文档路由与公开 API 表面纳入校验** - `/api/docs`、swagger bootstrap、`/api/openapi.json` 以及面向贡献者的工具路由现在都由 CI 直接检查。
- **API 文档已同步到 main 的真实路由表面** - `docs/api.md` 与 `docs/openapi.json` 已补充 task reports、project helpers、subtasks、agent spawn、announcements/directives、GitHub/OAuth/skills/sprites/update-auto 等路线。
- **E2E inbox 校验不再依赖本地敏感 secret** - `/api/inbox` 测试使用非敏感的测试专用 secret，而不是开发者本地 `.env`。

- 详细说明: [`docs/releases/v2.0.2.md`](docs/releases/v2.0.2.md)
