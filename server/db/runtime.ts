...
export function initializeDatabaseRuntime(): {
  ...
  if (!process.env.DB_PATH && !fs.existsSync(DEFAULT_DB_PATH) && fs.existsSync(LEGACY_DB_PATH)) {
    ...
    console.log("[Claw-Republic] Migrated database: claw-republic.sqlite -> claw-republic.sqlite");
  }
  ...
  console.log(
    `[Claw-Republic] SQLite write resilience: busy_timeout=${SQLITE_BUSY_TIMEOUT_MS}ms, ` +
  ...
  console.log(
    `[Claw-Republic] Review guardrails: max_rounds=${REVIEW_MAX_ROUNDS}, ` +
  ...
  console.log(
    `[Claw-Republic] In-progress watchdog: grace=${IN_PROGRESS_ORPHAN_GRACE_MS}ms, ` +
      `sweep=${IN_PROGRESS_ORPHAN_SWEEP_MS}ms`,
  );
  console.log(`[Claw-Republic] Subtask delegation sweep: interval=${SUBTASK_DELEGATION_SWEEP_MS}ms`);
